...
public interface FleetRepo extends JpaRepository<Fleet, Integer>{
    ...
    @Query(value = "SELECT f.id AS ID, f.fleet_name AS NAME, count(v.mmsi) AS NUMBER FROM fleet f, vessel v, fleet_has_ships c WHERE f.id = c.fleet_id AND v.mmsi = c.vessel_id AND f.user_id = :uid GROUP BY f.id", nativeQuery = true)
